...
if ($Phase -eq "controller") {
    ...
    $dseq = ($out | Select-String -Pattern '^\s*dseq\s+(\S+)' | Select-Object -First 1).Matches.Groups[1].Value
    $url  = ($out | Select-String -Pattern '^\s*url\s+(\S+)'  | Select-Object -First 1).Matches.Groups[1].Value
    if ($dseq) {
        @{ dseq = $dseq; url = $url; role = "controller" } |
