...
Provenance: a verbatim copy of v1's pz-controller/build_packages.py, kept in
Python and kept unedited on purpose. It is build-time SteamCMD wrangling that
already works, none of it runs in a container that serves players, and rewriting
it in Go would put the mod lists at risk for no runtime benefit. The copy exists
because pz-controller/ is deleted at cutover; the original is not maintained in
two places, so changes belong here and nowhere else.
"""
